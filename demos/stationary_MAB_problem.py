...
def train(env, agent, exploration_parameter, init_bias, episodes):
	''' Function trains the ε-Greedy agent on the environment '''

	# Initial parameters
	agent.reset_memory(exploration_parameter, init_bias)
	reward = 0
...
avg_rewards_for_over_num_runs_ucb_greedy = []
...
for i in range(num_runs):
	...
	avg_reward_for_ucb_greedy = train(env, ucb_greedy_agent, ucb_confidence, 
										init_bias, episodes)[0]
	avg_rewards_for_epsilons_over_num_runs_epsilon_greedy.append(
							avg_reward_for_epsilons_epsilon_greedy)
	avg_rewards_for_over_num_runs_ucb_greedy.append(
							avg_reward_for_ucb_greedy)


avg_rewards_for_epsilons_over_num_runs_epsilon_greedy = np.array(
						avg_rewards_for_epsilons_over_num_runs_epsilon_greedy)
avg_rewards_for_over_num_runs_ucb_greedy = np.array(
							avg_rewards_for_over_num_runs_ucb_greedy)
...
avg_rewards_for_over_num_runs_ucb_greedy = np.mean(
					avg_rewards_for_over_num_runs_ucb_greedy, axis=0).reshape(episodes,1)
...
plt.plot(avg_rewards_for_over_num_runs_ucb_greedy, label = "(UCB-Greedy) c = 2")
plt.legend(loc = "lower right") 
plt.show()
